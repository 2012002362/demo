...
class TestYeMian(unittest.TestCase):
    ...
    def test_denglu(self, username, password, status):
        ...
        if status == "0":
            ...
            sleep(3)
            AddEventcaozuo(self.driver).addname("哈哈哈")
            AddEventcaozuo(self.driver).add_address("金融港")
            AddEventcaozuo(self.driver).add_date("2023-10-30")
            AddEventcaozuo(self.driver).add_nums("10")
            AddEventcaozuo(self.driver).submit_btu()
            sleep(3)
            xinxi = self.driver.find_element(By.XPATH, '/html/body/div[1]/div[2]/form/div[6]/div/p')
            print(xinxi.text)
            self.assertIn(xinxi.text, "添加发布会成功！")
            sleep(3)
            self.driver.back()
            AddJiaBinCaoZuo(self.driver).jb()
            AddJiaBinCaoZuo(self.driver).tj()
            sleep(3)
            AddJiaBinCaoZuo(self.driver).fbh()
            AddJiaBinCaoZuo(self.driver).sjh("188888888")
            AddJiaBinCaoZuo(self.driver).Em("188888888@qq.com")
            AddJiaBinCaoZuo(self.driver).xm("马六")
